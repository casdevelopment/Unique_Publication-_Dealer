...
class CreateCatalogViewModel(private val repository: Repository) : ViewModel() {

    fun createCatalog(request: AddCatalogRequestModel): LiveData<NetworkStates<Response<BaseResponse<Any>>>> {
        return liveData(Dispatchers.IO) {
            emit(NetworkStates.loading(null))
            try {
                val response: Response<BaseResponse<Any>> = repository.createCatalog(request)
