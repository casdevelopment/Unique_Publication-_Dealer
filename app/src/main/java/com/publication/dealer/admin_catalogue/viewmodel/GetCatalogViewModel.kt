...
class GetCatalogViewModel(private val repository: Repository) : ViewModel() {

    fun getCatalog(): LiveData<NetworkStates<Response<BaseResponseCatalog<List<AddCatalogRequestModel>>>>>
    {
        return liveData(Dispatchers.IO) {
            emit(NetworkStates.loading(null))
            try {
                val response = repository.getCatalog() // repository should accept page param
