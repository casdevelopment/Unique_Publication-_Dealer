...
class BroadCastNotificationViewModel(private val repository: Repository) : ViewModel() {

    fun broadCastNotification(request: BroadCastRequestmodel): LiveData<NetworkStates<Response<BaseResponse<Boolean>>>> {
        return liveData(Dispatchers.IO) {
            emit(NetworkStates.loading(null))
            try {
                val response = repository.broadCastNotification(request)
